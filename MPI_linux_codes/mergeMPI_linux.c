...
int main(int argc, char** argv) {
	int *data, *chunk, *other, m, n=N, id, p, s = 0, i, step;
...
//(int *)malloc((n+s-r)*sizeof(int));
		...
		chunk = new int[s]; //(int *)malloc(s*sizeof(int));
	...
	while(step<p)
	{
		if(id%(2*step)==0)
		{
			if(id+step<p)
			{
				...
				other = new int[m]; // (int *)malloc(m*sizeof(int));
...
int * merge(int *A, int asize, int *B, int bsize) {
    ...
    if(C==NULL)
	   C = new int[csize]; //(int *)malloc(csize*sizeof(int));
